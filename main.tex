...
Here, we propose a core of  robust datasets that match optimal criteria for benchmarking but then we would consider incorporating new datasets which explore increasingly difficult concepts. A variety of parameters can guide future datasets.
...
In the case of chiral centers, the binding data should be available for the individual stereoisomers.
...
A successful free energy calculation requires a well prepared model of the system to be simulated, with structure(s) representative of the equilibrium state of the system. 
%
...
%
However, this metric only gives a theoretical limit and not the real quality of the model. Therefore it is not a good metric for quality and should only be used alongside other metrics. Iridium requests a resolution threshold of $< 3.5\,$\AA,\cite{warrenEssentialConsiderationsUsing2012} although more strict thresholds have been suggested (i.e. $<2.0\,$\AA  in a recent benchmark\cite{schindler_largescale_2020}).
%
...
\begin{table}[h]
\caption{
Evaluation of the quality of structural experimental data of a proposed benchmark set. 
%
...
DPI.
%
Additionally, a PDB ID of a higher quality structure and 
its quality measures are provided.
}
\label{tab:struct}
\input{struct_table}
\end{table}
...
The methods of binding free energy calculations have been continuously developed over the last decades and are increasingly used both in academic research as well as pharmaceutical industry applications in structure based drug discovery~\cite{sherborne_collaborating_2016, gapsysLargeScaleRelative2020,schindler_largescale_2020}. 
%
...
In order to benchmark methods reliably, we provide best practices recommendations for setting up benchmark calculations.
This setup begins with the appropriate choice of the experimental data, which includes 
the choice of target(s) and ligands (Section \ref{sec:dataset}) with both
structural information (Section \ref{sec:struct_data}) and 
affinity data (Section \ref{sec:affinities}.
This input data needs to be adequately prepared (Section \ref{sec:prep}) before the systems are simulated with a specific choice of 
software, 
calculation setup, and 
simulation protocol.
To finally assess the accuracy of the calculation, present the results and compare to other calculations, we establish standard reporting procedures which are presented in Section \ref{sec:analysis}.

Our learnings are exemplified in publically available tools for the analysis of calculations (github.com/openforcefield/arsenic)
and a living protein-ligand benchmark dataset (github.com/openforcefield/PLBenchmarks).
%
Note that our recommendations are in part not fulfilled in the living benchmark dataset. The main reason is the difficulty to find large and accurate experimental data sets to date. But the issues are annotated and the benchmark set should evolve to meet our recommendations.

We hope that this best practice recommendations will be adopted and extended by the community in order to advance the accuracy, applicability and availability of binding free energy calculations in the future. 
...
\begin{Checklists*}
\begin{checklist}{System Preparation}
\textbf{Prepare structural data for simulation: Section~\ref{sec:prep}}
    \begin{itemize}
        \item Assess which domains of the X-ray structure are needed, remove any parts that will not affect your simulation.
    ...
    \begin{itemize}
                ...
                \item If possible, model missing loops, if loops are too long (> 15 20 residues) or too mobile consider: capping the ends and add restraints, or model a short GLY loop that links both ends.
                \item Check side chain flips (HIS, ASN, GLN) – particularly that orientations are chosen leading to preferred interactions with the ligand
                \item Check the protonation states – again checking in the context of the interactions that would be formed with the ligand.
    \end{itemize}
    ...
    \begin{itemize}
            ...
            \item Check tautomer and ionizations states.
    \end{itemize}
        \item Assemble the protein and ligand, solvate solvate or embedded in a membrane. 
        \item Add ions.
        \item Equilibrate the system.
    \end{itemize}
\end{checklist}

\begin{checklist}{Simulation Setup}
